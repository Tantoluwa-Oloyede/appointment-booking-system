...
const validateRule = (rule, context = "set") => {
  const { day_of_week, status, start_time, end_time, break_start, break_end } =
    rule;
  ...
  try {
    ...
    if (!providerProfile) {
      return res.status(403).json({
        ...
        code: 403,
        message:
          "You must complete your business profile before setting availability",
      });
    }
    ...
    const saved = await Promise.all(
      rules.map((rule) =>
        availabilityModel.upsertAvailabilityRule({
...
export const updateAvailabilityRule = async (req, res, next) => {
  try {
    ...
    const providerProfile = await serviceModel.getProviderIdByUserId(user_id);
    if (!providerProfile) {
      return res.status(403).json({
        status: "error",
        code: 403,
    ...
    if (!updated) {
      return res.status(404).json({
        ...
        code: 404,
        message:
          "Availability rule not found or you do not have permission to update it",
      });
    }
...
export const getAvailableSlots = async (req, res, next) => {
  try {
    ...
    const rule = await availabilityModel.getAvailabilityRuleByDay(
      provider_id,
      dayOfWeek,
    );
    ...
    const breakStart = rule.break_start
      ? timeToMinutes(rule.break_start)
      : null;
    ...
    while (current + duration <= workEnd) {
      ...
      const overlapsBreak =
        breakStart !== null && slotStart < breakEnd && slotEnd > breakStart;

      if (!overlapsBreak) {
        const slotStartISO = new Date(
          `${date}T${minutesToTime(slotStart)}:00.000Z`,
        ).toISOString();
        const slotEndISO = new Date(
          `${date}T${minutesToTime(slotEnd)}:00.000Z`,
        ).toISOString();
        ...
        const isBooked = bookedPeriods.some((b) => {
          const bStart = new Date(
            b.booking_period.start ?? b.booking_period,
          ).getTime();
          const bEnd = new Date(
            b.booking_period.end ?? b.booking_period,
          ).getTime();
        ...
        const isBlocked = providerBlocks.some((b) => {
          const bStart = new Date(
            b.block_period.start ?? b.block_period,
          ).getTime();
